...
这份文档不仅涵盖了功能说明，还重点解释了脚本中的自动化逻辑（如配额监控和节点切换）。

---

# OpenClaw on Hugging Face (with Tailscale Exit Node)
...
* **配额超限保护**：实时监控 OpenClaw 日志。一旦触发 `429 Quota Exceeded` 错误，脚本会自动清理进程、更换模型并重启。
...
程序主要由 `start-openclaw.sh` (或你的启动脚本) 驱动，包含以下几个并行模块：
...
1.  **节点授权**：请务必在 Tailscale 控制面板中手动将你的 PC/手机设置为 **"Run as Exit Node"**，并关闭关键节点的 Key Expiry。
2.  **存储空间**：Hugging Face 的本地存储是临时性的。请确保 `sync.py` 正常工作，以便将数据持久化到你的 Private Dataset 或 Repository。
3.  **网络安全**：OpenClaw 已配置为本地模式并开启 Token 校验，但请勿泄露你的网关密码。
...
1.  在 Hugging Face 上创建一个新的 Space (选择 Docker 模板)。
2.  将你的代码上传至仓库。
3.  在 **Settings -> Variables and secrets** 中配置上述所有环境变量。
4.  保存并等待自动构建。
